  ...
  try {
    ...
    return (
      <div className="flex justify-center items-center">
        <div className={`w-${width} h-${height} relative rounded-md overflow-hidden bg-gray-100`}>
          <img
            src={imageUrl}
            alt={alt}
...
export function renderPrice<T>(
  ...
  try {
    ...
    const originalPrice = originalPriceAccessorKey
      ? (row.getValue(originalPriceAccessorKey) as number) || 0
      : 0;

    return (
      <div className="flex flex-col items-end text-right">
        <span className="font-medium">{formatFunction(price)}</span>
        {originalPriceAccessorKey && price < originalPrice && (
...
export function renderDate<T>(
  ...
  try {
    ...
    return <div className={`text-${alignment} ${alignment === "right" ? "pr-4" : ""}`}>{formattedDate}</div>;
  } catch (error) {
    console.error("Error rendering date:", error);
    return <div className={`text-${alignment} ${alignment === "right" ? "pr-4" : ""}`}>N/A</div>;
  }
}
...
export function renderText<T>(
  ...
  try {
    ...
    return <div className={`text-${alignment}`}>{text || "N/A"}</div>;
  } catch (error) {
